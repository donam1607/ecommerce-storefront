      ...
      <section id="products" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20">
            ...
            <button onClick={() => setSearch("")} className="mt-4 text-blue-600 hover:underline font-semibold">
              View all products
            </button>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filtered.map((product) => (
              <div
                key={product.id}
                className="group bg-white border border-slate-200 rounded-2xl overflow-hidden hover:shadow-xl hover:shadow-slate-200/60 hover:-translate-y-1 hover:border-blue-200 transition-all duration-300 flex flex-col"
              >
                {/* Image */}
                ...
                <div className="p-4 flex-grow flex flex-col justify-between">
                  <div>
                    ...
                    <Link to={`/product/${product.id}`}>
                      <h3 className="font-bold text-slate-800 mt-1 group-hover:text-blue-600 transition-colors line-clamp-1 text-sm">
                        {product.name}
                      </h3>
                    </Link>
                    ...
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-3 py-2 rounded-xl font-bold text-xs transition-all ${
                        addedId === product.id
                          ...
                          : "bg-blue-600 hover:bg-blue-500 text-white hover:shadow-md hover:shadow-blue-600/20 active:scale-95"
                      }`}
                    >
      ...
      <section className="bg-slate-900 text-white py-16 px-4 mt-8">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
          <div className="p-6">
            ...
            <p className="text-slate-400 text-sm">On all orders over $150. Express delivery available.</p>
          </div>
          <div className="p-6">
            <div className="text-4xl mb-3">🛡️</div>
            <h3 className="font-bold text-lg mb-2">2-Year Warranty</h3>
            <p className="text-slate-400 text-sm">All products come with full manufacturer warranty.</p>
          </div>
          <div className="p-6">
            <div className="text-4xl mb-3">↩️</div>
            <h3 className="font-bold text-lg mb-2">30-Day Returns</h3>
            <p className="text-slate-400 text-sm">Not satisfied? Return it within 30 days, no questions asked.</p>
          </div>
        </div>
      </section>
    </div>
